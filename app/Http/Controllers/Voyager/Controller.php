...
class Controller extends VoyagerBreadController {
        ...
        switch ($row->type) {
            ...
            case 'image':
                if ($request->hasFile($row->field)) {
                    ...
                    $object_image = $row->field;
                    $image_type = 'public';
                    if($object_image == 'signature') {
                        $image_type = '';
                    }
                    Storage::disk(config('voyager.storage.disk'))->put($fullPath, file_get_contents($request->$object_image), $image_type);
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($slug == 'posts' && $row->type == 'select_multiple' && $row->field == 'post_tag'){
            \App\Post::find($data->id)->{$row->field}()->sync($multi_select[0]['content']);
        }
        else {
