                ...
                <div class="col-md-4">
                    ...
                    <div class="panel panel panel-bordered panel-warning">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="name">Post Categories</label>
                                <select class="form-control" name="category_name">
                                    @foreach (App\Post::ALL_CATEGORIES as $cat)
                                        <option value="{{ $cat }}" @if(isset($dataTypeContent->category_name) && $dataTypeContent->category_name == $cat){{ 'selected="selected"' }}@endif>{{ $cat }}</option>
                                    @endforeach
                                </select>
                            </div>
`                            @php
                                $tags = \App\Post::find($dataTypeContent->id);
                                if($tags) {
                                    $tags = $tags->post_tag()->pluck('id')->toArray();
                                }
                            @endphp
                            <div class="form-group">
                                <label for="name">Post Tags</label>
                                <select class="form-control select2" name="post_tag[]" multiple>
                                    @foreach(\App\PostTag::all() as $tag)
                                        <option value="{{ $tag->id }}" @if(isset($tags) && in_array($tag->id, $tags)){{ 'selected="selected"' }}@endif>{{ $tag->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="panel panel-bordered panel-primary">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="icon wb-image"></i> Post Image (275x183)</h3>
                            <div class="panel-actions">
                                <a class="panel-action voyager-angle-down" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
