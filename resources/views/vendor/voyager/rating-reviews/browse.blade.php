        ...
        <h1 class="page-title">
            ...
            <div class="row">
                @if(Voyager::can('edit_'. $dataType->name))
                    <button class="btn btn-success btn-action-new btn-approve-rating-review disabled" data-type="approve" style="margin-right:2rem;">
                        Accept
                    </button>
                    <button class="btn btn-danger btn-action-new btn-reject-rating-review disabled" data-type="reject">
                        Reject
                    </button>
                @endif
            </div>
...
@section('javascript')
    ...
    <script>
        ...
        $(document).on('click', 'input[name=checkbox]', function (e) {
            var self = $(this);
            var index = ids.indexOf(self.val());
            if(self.is(':checked')) {
                if(index === -1) {
                    ids.push(self.val());
                }
            ...
            if(ids.length > 0) {
                $('.btn-action-new').removeClass('disabled');
            } else {
                $('.btn-action-new').addClass('disabled');
            }
        });

        $(document).on('click', '.btn-action-new', function (e) {
            e.preventDefault();
            var self = $(this);
            if(!self.hasClass('disabled')) {
                handleApproveRejectAction(self.data('type'));
            }
        });

        function handleApproveRejectAction(type) {
            $('.btn-action-new').addClass('disabled');
            ...
            $.ajax({
                url: "{{ route('admin.rating-review.bulk.update') }}",
                type: 'POST',
                async: true,
                data: {
                    type: type,
                    rating_review_id: ids,
                    _token: "{{ csrf_token() }}"
                },
